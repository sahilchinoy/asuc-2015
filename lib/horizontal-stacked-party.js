...
var cb = ['rgb(178,223,138)','rgb(31,120,180)','rgb(251,154,153)','rgb(166,206,227)','rgb(51,160,44)','rgb(251,154,153)']
        ...
        d3.csv(GRAPHIC_DATA_URL, function(error, data) {
            ...
            graphicData.forEach(function(d) {
                ...
                color.domain().map(function(name) {
                    d['value'].push({ 'label': name, 'amt': +d[name], 'x0': x0});
                    x0 += +d[name];
                    delete d[name];
                });
                delete d['Group'];
...
var drawGraph = function(graphicWidth) {
    ...
    var groupHeight = BAR_HEIGHT;
    var ticksX = 7;

    // define chart dimensions
    var width = graphicWidth - margin['left'] - margin['right'];
    var height = ((BAR_HEIGHT + BAR_GAP) * numGroups) - BAR_GAP + BAR_GAP_INNER;
